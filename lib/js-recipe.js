...
function recipe (config) {
  ...
  // Create dev task:
  gulp.task(name, deps, dev(src, out, buildDest, options));

  // Create compile task:
  gulp.task(name + '-compile', deps, compile(src, out, compileDest, options));

  // Create hint task:
  gulp.task(hintTask, hint(src, out, hintOptions, hintTask));
}
...
function jsBase (src, out, options) {
  ...
  if (options.browserify) {
    ...
    // TODO handle watch callbacks.  Can do this in the recipe return
    // statement somehow?
