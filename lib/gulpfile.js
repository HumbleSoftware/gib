...
module.exports = function (config, gulp) {
  ...
  var lessTasks = [];
  var lessCompileTasks = [];
  _.each(config.less, function (src, out) {
    var options = _.isObject(src) ? src : {};
    var src = src.src || src;
    var name = 'less-' + out;
    var nameCompile = 'less-compile-' + out;
    lessTasks.push(name);
    lessCompileTasks.push(name);
    watches.push({
      src: options.watch || src,
      tasks: [name]
    });
    gulp.task(name, function () {
      return less(src, out, options)
        .pipe(gulp.dest(config.build))
        .pipe(gulpConnect.reload());
    });
    gulp.task(nameCompile, function () {
      return less(src, out, _.extend({cleancss: true}, options))
        .pipe(gulp.dest(config.compile || config.build ))
        .pipe(gulpConnect.reload());
    })
  });
  gulp.task('less', lessTasks);
  gulp.task('less-compile', lessCompileTasks);
